...
def _get_closed_pnl_sync(ticket: int) -> tuple[float, float | None]:
    if not MT5_AVAILABLE:
        return 0.0, None
    try:
        # Utilisez l'heure locale au format timezone UTC pour que MT5 fasse bien la conversion
        # Ou plus simple : récupérez l'heure du dernier tick MT5
        time_to = datetime.now() + timedelta(days=1)
        time_from = datetime.now() - timedelta(days=3) # Visez très large (3 jours) pour absorber tous les décalages horaires
        deals = mt5.history_deals_get(time_from, time_to)
        
        if not deals:
            return 0.0, None
        for deal in reversed(deals):
            if deal.position_id == ticket and deal.entry == mt5.DEAL_ENTRY_OUT: # Correction ici : DEAL_ENTRY_OUT
